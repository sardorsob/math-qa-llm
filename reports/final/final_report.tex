...
\subsection{Results}
\label{sec:results}
\textcolor{red}{Present the results from \new{your final solution}. This can be a mix of quantitative results (e.g., validation metrics or leaderboard score) and qualitative results (e.g., example visualizations of predictions, case studies, etc.). Use figures and tables. You are welcome to include any negative findings/results.}
\paragraph{Result 1}The first completed public-set test evaluated Baseline A on 10 randomly selected public examples. The purpose of this experiment was to verify that the full pipeline could load questions, generate responses, extract answers, score public examples, and save the resulting JSONL artifact.
...
\paragraph{Result 2}
% etc.
Table 2 compares the generic single-pass baseline, the structured single-pass baseline, and the adaptive pipeline on the same 50-question public validation subset.
...
\begin{table}[h]
    ...
    \begin{tabular}{p{3.2cm}ccc}
        \hline
        \textbf{Approach} 
        & \shortstack{\textbf{MCQ} \\ \textbf{accuracy}} 
        & \shortstack{\textbf{Free-form} \\ \textbf{accuracy}} 
        & \shortstack{\textbf{Overall} \\ \textbf{accuracy}} \\
        \hline
        Generic prompt, single pass 
        & \text{[INSERT]} 
        & \text{[INSERT]} 
        & \text{[INSERT]} \\
...
Compared with the generic single-pass baseline, the structured prompt changes only the prompt instructions. Therefore, the difference between the first two rows isolates the effect of the structured reasoning format. The comparison between the structured single-pass baseline and the adaptive pipeline measures the value of selectively retrying uncertain questions.

After running the experiment, report improvements using both absolute counts and percentage points. For example:

The adaptive pipeline improved overall accuracy from [A]\% to [B]\%, an increase of [B - A] percentage points, while increasing the average inference time from [C] to [D] seconds per question. The improvement was [larger/smaller] for free-form questions than for MCQ questions. The number of responses truncated by the length limit decreased from [E] to [F].

To visualize the computational tradeoff, Figure \ref{fig:reasoning_pipeline} should show the number of examples that terminate after Phase 1, escalate to Phase 2, and escalate to Phase 3.
