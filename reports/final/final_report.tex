...
We implemented our inference pipeline and training procedures using PyTorch and HuggingFace Transformers, migrating from vLLM due to CUDA 12.8/13 compatibility issues on the DSMLP cluster. All experiments and model tuning were conducted on a DSMLP environment using a single NVIDIA A30 GPU (24GB VRAM).

\paragraph{Inference Details}
The adaptive inference pipeline consisted of two phases. Phase 1 used a maximum generation length of 4096 tokens (1024 reserved for thinking) with greedy decoding. Phase 2 allocated 5120 tokens (4096 thinking) using temperature $T=0.7$, Top-$P=0.9$, and Top-$K=50$ for majority voting. Generating one candidate per question for the 943-question private set took approximately 10 hours on an A30 GPU using batched inference with scaled dot-product attention (SDPA).

\paragraph{Training Details}
For supervised fine-tuning via QLoRA, we used the AdamW optimizer with a learning rate of $5 \times 10^{-5}$ and a cosine learning rate scheduler. The model was trained for 3 epochs with a batch size of 4, utilizing 4-bit quantization and a sequence length of 8192 to accommodate long reasoning traces. 
For reinforcement learning via GRPO, we reduced the group size to $G=8$ to fit within the 24GB VRAM constraint, using a KL penalty coefficient of $\beta=0.1$ and a learning rate of $1 \times 10^{-6}$.
% 
\subsection{Results}
\label{sec:results}
...
To visualize the computational tradeoff, Figure 1 should show the number of examples that terminate after Phase 1, escalate to Phase 2, and escalate to Phase 3.
