...
## 2026-05-31 — Pipeline completion + heuristic recovery

### Final inference state

| Metric | Value |
|---|---|
| Total private questions | 943 |
| Phase 1 responses (single shot) | 584 |
| Phase 2 responses (EBM-reranked) | 359 |
| Phase 3 | DISABLED (USE_PHASE3=False) |
| Responses with `\boxed{}` after inference | 491 (52.1%) |
| Responses with `finish_reason="length"` | 500 (53.0%) |
| MCQ truncation rate (no \\boxed{}) | 73.7% (221/300) — disaster |
| FF truncation rate (no \\boxed{}) | 35.9% (231/643) |

The truncation rate is the dominant accuracy killer. Phase 2 was actually WORSE than Phase 1 for truncation (73.0% vs 32.5%) because Phase 2 questions are inherently harder — the model thinks longer on them and still hits the cap.

### Heuristic recovery — what was added

Two scripts in `scripts/`:

| Script | Purpose | Coverage |
|---|---|---|
| `scripts/jsonl_to_csv.py` | Convert checkpoint JSONL → competition CSV (mirrors Section 7 of nb06) | 943/943 rows |
| `scripts/recover_truncated_answers.py` | v1 recovery — regex extraction of answer signals from truncated responses | 491 → 938 boxed (99.5%) |
| `scripts/recover_truncated_answers_v2.py` | v2 — adds "should be X" patterns, `= X` non-anchored, MCQ option-count validator, multi-answer delimiter detection, repeated-value confirmation | 491 → 941 boxed (99.8%) + `--compare-with` mode |

### Kaggle score progression

| Submission | CSV | Score |
|---|---|---|
| Initial (Phase 1+2 raw) | `submission_qlora_v1_merged_2026-05-31.csv` | ~45% |
| v1 recovery | `submission_qlora_v1_merged_2026-05-31_recovered.csv` | ~52% |
| v2 recovery | `submission_qlora_v1_merged_2026-05-31_recovered_v2.csv` | TBD (submitted) |

### Files generated by this session

```
artifacts/logs/runs/private_qlora_v1_merged_checkpoint.jsonl     ← raw inference output
artifacts/logs/runs/private_qlora_v1_merged_results.jsonl        ← metadata-enriched
artifacts/logs/runs/private_qlora_v1_merged_recovered_results.jsonl    ← v1 recovery
artifacts/logs/runs/private_qlora_v1_merged_recovered_v2_results.jsonl ← v2 recovery
artifacts/submissions/submission_qlora_v1_merged_2026-05-31.csv
artifacts/submissions/submission_qlora_v1_merged_2026-05-31_recovered.csv
artifacts/submissions/submission_qlora_v1_merged_2026-05-31_recovered_v2.csv
```

### Why recovery instead of re-running with bigger `MAX_TOKENS`

- Phase 1 ran with `MAX_TOKENS=4096`, Phase 2 with `5120` — both too tight for thinking model
- Re-running with `8192`+ would solve truncation but takes 1-2 more days of GPU compute
- Heuristic recovery does the same job in ~10 seconds without GPU
- Trade-off: recovery accuracy is lower than re-running (model produces actual answers), but the time budget didn't allow re-running

### Key learning for future projects

**Token budget is the dominant lever for thinking-model accuracy.** Self-consistency (more samples), EBM reranking, and prompt engineering each contribute single-digit % gains. Letting the model finish its thinking trace contributes 10-15% by itself. For Qwen3-4B-Thinking, set `PHASE1_MAX_TOKENS ≥ 8192` and `PHASE2_MAX_TOKENS ≥ 10240` from the start — even at the cost of longer wall-clock time.
